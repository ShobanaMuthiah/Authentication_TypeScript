...
exports.chatMessages = exports.userChatList = void 0;
const data_source_1 = require("../db/data-source");
const chatModels_1 = require("../db/Models/chatModels");
...
// }
const userChatList = async (req, res, next) => {
    const userId = req.query.userId;
    console.log("userId: ", req.query.userId);
    const chats = await data_source_1.AppDataSource.getRepository(chatModels_1.Chats)
        .createQueryBuilder("chats")
        .select([
        "MIN(chats.id) as id",
...
const chatMessages = async (req, res, next) => {
    const { userId, receiverId } = req.body;
    console.log("user id ", userId, receiverId);
    if (!userId || !receiverId)
        throw new exception_util_1.HTTPError(400, "Require both send ID and receiver ID");
    let chats = await data_source_1.AppDataSource.getRepository(chatModels_1.Chats)
        .createQueryBuilder("chats")
